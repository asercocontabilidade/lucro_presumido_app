...
exports.consolidarAnual = consolidarAnual;
...
async function consolidarAnual(empresaId, ano) {
    const where = { ano };
    ...
    const trimestres = calculos.map((c, idx) => {
        ...
        if (idx > 0) {
            ...
            if (vRec) {
                t.variacaoReceita = vRec.abs;
                t.variacaoReceitaPct = vRec.pct;
            }
            if (vIrpj) {
                t.variacaoIrpj = vIrpj.abs;
                t.variacaoIrpjPct = vIrpj.pct;
            }
            if (vCsll) {
                t.variacaoCsll = vCsll.abs;
                t.variacaoCsllPct = vCsll.pct;
            }
        }
        return t;
    });
