...
const LIMITE_MAJORACAO = 1_250_000;
const LIMITE_ADICIONAL = 60_000;
const LIMITE_ADICIONAL_MENSAL_PDF = 20_000;

const MESES_TRI_PDF: Record<number, [string, string, string]> = {
...
const LOGO_PATHS = [
  ...
  resolve(__dirname, '../../../frontend/public/LOGO.png'),
  resolve(__dirname, '../../LOGO.png'),
];
const LOGO_PATH = LOGO_PATHS.find(p => existsSync(p)) ?? null;
...
interface LinhaFaixa {
  descricao: string; aliquota: number; aliquotaAcrescimo: number;
  valor: number; proporcao: number; parcelaSemAcrescimo: number;
  parcelaComAcrescimo: number; baseCalculo: number;
}

interface CalculoParaPdf {
  ano: number; trimestre: number; descricao?: string | null;
  criadoEm: Date | string;
  receitaTotal: number; excedenteMajorado: number;
  basePresumidaIrpj: number; irpj15: number; adicionalIr10: number;
  irpjTotal: number; irrf: number; irpjARecolher: number;
  basePresumidaCsll: number; csll9: number; csllRetida: number; csllARecolher: number;
  detalheCalculo: Record<string, unknown>;
  usuarioCriacao: { nome: string };
  empresa?: { razaoSocial: string; cnpj?: string | null; nomeFantasia?: string | null } | null;
  /** Calculos do mesmo exercicio para comparativo (opcional) */
  comparativoExercicio?: {
    trimestre: number; receitaTotal: number;
    irpjARecolher: number; csllARecolher: number;
  }[];
  /** Receitas mensais para linha do tempo (modalidade mensal) */
  receitasMensais?: Array<{
    receita16: number; receita8: number; receita16p: number;
    receita32: number; outrasReceitas: number; irrf: number; csllRetida: number;
  }>;
}
...
export function gerarPdf(calculo: CalculoParaPdf): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    const detalhe       = calculo.detalheCalculo as Record<string, unknown>;
    const linhasIrpj    = (detalhe?.linhasIrpj  as LinhaFaixa[]) ?? [];
    const linhasCsll    = (detalhe?.linhasCsll  as LinhaFaixa[]) ?? [];
    ...
    const hCsllCard = renderCardResultado(doc, L + cardW + 12, ry, cardW, linhasCsllCard,
      csllBloqueada ? 'CSLL - DARF 2372  (Sem majoracao 1T/2026)' : 'CSLL - DARF 2372');
    ...
    if (temPagMensais) {
      ...
      // Montar linhas IRPJ
      const linhasResidualIrpj: { label: string; value: string; color: string; bold: boolean }[] = [
      ...
      // Montar linhas CSLL
      const linhasResidualCsll: { label: string; value: string; color: string; bold: boolean }[] = [
    ...
    if (modalidade === 'mensal' && calculo.receitasMensais && calculo.receitasMensais.length >= 3) {
      ...
      // Calcular valores dos meses 1 e 2 (antecipacao — sem majoracao)
      interface DadosMes {
        nome: string; receitaBruta: number;
        baseIrpj: number; irpj15: number; adicional: number; irpjRecolher: number;
        baseCsll: number; csllRecolher: number;
        fechamento: boolean;
      }

      function calcMesAntecipacao(m: typeof rm[0], nomeMes: string): DadosMes {
      ...
      function renderLinhaMes(dados: DadosMes, idx: number) {
    ...
    const notas: { titulo: string; texto: string; cor: string; corBarra: string }[] = [
      {
    ...
    for (let i = 0; i < notas.length; i += 2) {
      ...
      if (notaDir) renderNota(doc, L + notaW + 10, rowY, notaW, rowH, notaDir);
      doc.y = rowY + rowH + 6;
    }
    ...
    doc.fillColor(C.muted).fontSize(7).font('Helvetica')
      .text(
        `Documento gerado pelo Sistema de Conferencia de Majoracao - Aserco Contabilidade   |   Calculado em ${criadoEmStr} por ${safe(calculo.usuarioCriacao.nome)}   |   Exportado em ${exportEmStr}`,
        L + 8, sy + 5, { width: PW - 16, align: 'center' }
      );
    ...
    for (let i = 0; i < range.count; i++) {
      ...
      doc.fillColor('rgba(255,255,255,0.55)').fontSize(7).font('Helvetica')
        .text(
          `Sistema de Conferencia de Majoracao - Aserco Contabilidade   |   Lucro Presumido - ${TRIMESTRES_LABEL[calculo.trimestre]} ${calculo.ano}   |   Documento gerado automaticamente   |   Pagina ${i + 1} de ${range.count}`,
          L, FOOTER_Y + 10, { width: PW, align: 'center' }
        );
...
function paginaSeNecessario(doc: PDFKit.PDFDocument, minH: number) {
...
function calcNotaH(doc: PDFKit.PDFDocument, w: number, titulo: string, texto: string): number {
  const PAD = 7;
...
function renderNota(
  ...
  nota: { titulo: string; texto: string; cor: string; corBarra: string }
) {
  const PAD = 7;
...
function renderSecaoLabel(doc: PDFKit.PDFDocument, x: number, w: number, label: string) {
  const y = doc.y;
...
function renderResumoExecutivo(
  ...
  itens: { label: string; value: string; destaque: boolean }[]
) {
  const cols = 4;
  ...
  itens.forEach((item, i) => {
    ...
    if (item.destaque) doc.rect(ix, iy, 3, itemH).fill(C.red);
...
function renderTabelaComparativo(
  doc: PDFKit.PDFDocument, x: number, w: number,
  dados: { trimestre: number; receitaTotal: number; irpjARecolher: number; csllARecolher: number }[],
  trimestreAtual: number
) {
  const cols = [
    { label: 'Periodo',          cw: 100, align: 'left'  as const },
    { label: 'Receita Total',    cw: 130, align: 'right' as const },
    { label: 'IRPJ a Recolher',  cw: 120, align: 'right' as const },
    { label: 'CSLL a Recolher',  cw: 120, align: 'right' as const },
    { label: 'Total IRPJ+CSLL',  cw: 120, align: 'right' as const },
    { label: 'Var. IRPJ vs ant.', cw: 100, align: 'right' as const },
    { label: 'Var. %',           cw: 80,  align: 'right' as const },
  ];
  ...
  dados.forEach((d, idx) => {
    ...
    if (isAtual) doc.rect(x, cy, 3, ROW_H).fill(C.blue);
...
function renderTabelaReceitas(
  doc: PDFKit.PDFDocument,
  x: number, w: number,
  linhas: LinhaFaixa[],
  opts: { receitaTotal: number; excedente: number; baseTotal: number; houveExcedente: boolean; csllBloqueada: boolean }
) {
  const cols = [
    { label: 'Discriminacao das Receitas', w: 220, align: 'left'  as const },
    { label: 'Valor (R$)',                 w: 90,  align: 'right' as const },
    { label: 'Proporcao',                  w: 52,  align: 'right' as const },
    { label: 'Parc. s/ Acrescimo',         w: 90,  align: 'right' as const },
    { label: '% Aplic.',                   w: 48,  align: 'right' as const },
    { label: 'Parc. c/ Acrescimo',         w: 90,  align: 'right' as const },
    { label: '% c/ Acresc.',               w: 48,  align: 'right' as const },
    { label: 'Base de Calculo',            w: 132, align: 'right' as const },
  ];
  ...
  function desenharCabecalho(cy: number) {
    let cx = x;
    doc.rect(x, cy, w, HEAD_H).fill(C.navy);
    cols.forEach(col => {
      doc.fillColor(C.white).fontSize(7).font('Helvetica-Bold')
...
function renderCardResultado(
  doc: PDFKit.PDFDocument,
  x: number, y: number, w: number,
  linhas: { label: string; value: string; bold?: boolean; muted?: boolean; total?: boolean; highlight?: boolean }[],
  titulo: string
): number {
  const PAD    = 10;
  const LINE_H = 16;
  ...
  linhas.forEach(l => {
    ...
    if (isTotal) doc.rect(x, ly - 2, w, LINE_H + 2).fill('#112240');
...
function renderCardResidual(
  doc: PDFKit.PDFDocument,
  x: number, y: number, w: number, h: number,
  linhas: { label: string; value: string; color: string; bold: boolean }[],
  titulo: string,
  bgColor: string, headerColor: string, totalBg: string
) {
  ...
  let ly = y + 18;
  linhas.forEach((l, i) => {
    if (i === linhas.length - 1) doc.rect(x, ly - 2, w, 18).fill(totalBg);
