...
const LIMITE_ADICIONAL_MENSAL_XL = 20_000;

const MESES_TRI_XL_CONST: Record<number, [string, string, string]> = {
...
const NAVY:  ExcelJS.Fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF0A1628' } };
const BLUE:  ExcelJS.Fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF0D47A1' } };
const BLUEL: ExcelJS.Fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFE8EFF8' } };
const TOTAL: ExcelJS.Fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFEEF2F8' } };
const ALT:   ExcelJS.Fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFF7F9FC' } };
const REDL:  ExcelJS.Fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFFEE2E2' } };
const AMBL:  ExcelJS.Fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFFFF3CD' } };
const DARK2: ExcelJS.Fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF112240' } };

const WHITE_FONT:  Partial<ExcelJS.Font> = { color: { argb: 'FFFFFFFF' } };
const NAVY_FONT:   Partial<ExcelJS.Font> = { color: { argb: 'FF0A1628' } };
const BLUE_FONT:   Partial<ExcelJS.Font> = { color: { argb: 'FF0D47A1' } };
const RED_FONT:    Partial<ExcelJS.Font> = { color: { argb: 'FFB91C1C' } };
const MUTED_FONT:  Partial<ExcelJS.Font> = { color: { argb: 'FF8A93A6' } };
const LIGHT_FONT:  Partial<ExcelJS.Font> = { color: { argb: 'FFB0C4DE' } };
...
function border(style: ExcelJS.BorderStyle = 'thin', color = 'FFD8DDE8'): Partial<ExcelJS.Borders> {
  const s = { style, color: { argb: color } };
  return { top: s, bottom: s, left: s, right: s };
}

export async function gerarExcel(calculo: CalculoParaExcel): Promise<Buffer> {
  ...
  const MESES_TRI_XL: Record<number, [string, string, string]> = {
  ...
  hIrpj.fill = BLUE; hIrpj.font = { ...WHITE_FONT, bold: true, size: 10 }; hIrpj.alignment = CENTER;
  ws.getRow(r).height = 18;

  ws.mergeCells(r, 5, r, 8);
  const hCsll = ws.getCell(r, 5);
  hCsll.value = csllBloqueada ? 'CSLL — DARF 2372  ·  Sem majoração (1T/2026)' : 'CSLL — DARF 2372';
  hCsll.fill = NAVY; hCsll.font = { ...WHITE_FONT, bold: true, size: 10 }; hCsll.alignment = CENTER;
  r++;

  const irpjRows: { label: string; value: number; bold?: boolean; muted?: boolean; total?: boolean; highlight?: boolean }[] = [
  ...
  const csllRows: { label: string; value: number; bold?: boolean; muted?: boolean; total?: boolean }[] = [
  ...
  if (temPagMensais) {
    ...
    hIrpjR.fill = NAVY; hIrpjR.font = { ...WHITE_FONT, bold: true, size: 10 }; hIrpjR.alignment = CENTER;
    ws.mergeCells(r, 5, r, 8);
    const hCsllR = ws.getCell(r, 5);
    hCsllR.value = 'CSLL — DARF 2372';
    hCsllR.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF112240' } };
    hCsllR.font = { ...WHITE_FONT, bold: true, size: 10 }; hCsllR.alignment = CENTER;
    ws.getRow(r).height = 18;
    r++;

    interface LinhaResidual { label: string; value: number; deducao?: boolean; total?: boolean; credito?: boolean }
    const residualIrpj: LinhaResidual[] = [
    ...
    const residualCsll: LinhaResidual[] = [
    ...
    for (let i = 0; i < maxResidualRows; i++) {
      ...
      if (il) {
        ...
        lc.alignment = LEFT; lc.border = border('hair', 'FF1E3A5F');
        ...
        vc.alignment = RIGHT; vc.border = border('hair', 'FF1E3A5F');
      }
      ...
      if (cl) {
        ...
        lc2.alignment = LEFT; lc2.border = border('hair', 'FF1E3A5F');
        ...
        vc2.alignment = RIGHT; vc2.border = border('hair', 'FF1E3A5F');
      }
  ...
  interface NotaLegal { titulo: string; texto: string; fillArgb: string; fontArgb: string }
  const notas: NotaLegal[] = [];
  ...
  if (modalidadeXl === 'mensal' && calculo.receitasMensais && calculo.receitasMensais.length >= 3) {
    ...
    for (let ci = 2; ci <= 8; ci++) wlt.getColumn(ci).width = 16;
    ...
    // Calcular dados dos meses
    interface DadosMesXl {
      nome: string; receitaBruta: number;
      baseIrpj: number; irpj15: number; adicional: number; irpjRecolher: number;
      baseCsll: number; csllRecolher: number; fechamento: boolean;
    }

    function calcMesAntXl(m: typeof rm[0], nomeMes: string): DadosMesXl {
    ...
    ltMeses.forEach((m, idx) => {
      const bg = m.fechamento ? BLUEL : (idx % 2 === 0 ? { type: 'pattern' as const, pattern: 'solid' as const, fgColor: { argb: 'FFFFFFFF' } } : ALT);
      const fontBase: Partial<ExcelJS.Font> = m.fechamento
...
function renderSecaoLabel(ws: ExcelJS.Worksheet, r: number, cols: number, label: string): number {
...
function renderTabelaReceitas(
  ...
  linhas.forEach((l, idx) => {
    const bg = idx % 2 === 0 ? { type: 'pattern' as const, pattern: 'solid' as const, fgColor: { argb: 'FFFFFFFF' } }
                             : ALT;
    const temExcedente = opts.houveExcedente && l.parcelaComAcrescimo > 0;
    ws.getRow(r).height = 15;

    const cells: { value: string | number; numFmt?: string; font?: Partial<ExcelJS.Font>; align?: Partial<ExcelJS.Alignment> }[] = [
  ...
  totalVals.forEach((v, i) => {
    ...
    cell.alignment = i === 0 ? LEFT : RIGHT;
    cell.border = border('thin', 'FFB0BDD0');
    if (i === 1 || i === 3 || i === 5 || i === 7) cell.numFmt = BRL;
    if (i === 2) cell.numFmt = PCT;
  });
  r++;
