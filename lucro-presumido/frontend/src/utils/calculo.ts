...
/** Controla regras de período */
export interface OpcaoCalculo {
  /**
   * Quando true (padrão), aplica o acréscimo de 10% na CSLL sobre o excedente.
   * Deve ser FALSE para 1T/2026: MP 1.262/2024 — CSLL majorada vigente somente a partir do 2T/2026.
   */
...
export interface ResultadoCalculo {
  ...
  /** Indica que a CSLL foi calculada SEM o acréscimo de majoração (regra 1T/2026) */
  csllMajoracaBloqueada: boolean;
  /** Pagamentos de IRPJ já efetuados nos meses anteriores do trimestre */
  irpjMesesAnteriores: number;
  /** Pagamentos de CSLL já efetuados nos meses anteriores do trimestre */
  csllMesesAnteriores: number;
  /** IRPJ residual da guia do 3º mês = irpjARecolher − irpjMesesAnteriores */
  irpjResidual: number;
  /** CSLL residual da guia do 3º mês = csllARecolher − csllMesesAnteriores */
  csllResidual: number;
  /** Indica se há pagamentos mensais anteriores informados */
  ...
  /** Antecipação IRPJ do 1º mês do trimestre (empresa mensal) */
  irpjAntecipacaoMes1?: number;
  /** Antecipação IRPJ do 2º mês do trimestre (empresa mensal) */
  irpjAntecipacaoMes2?: number;
  /** Antecipação CSLL do 1º mês do trimestre (empresa mensal) */
  csllAntecipacaoMes1?: number;
  /** Antecipação CSLL do 2º mês do trimestre (empresa mensal) */
  csllAntecipacaoMes2?: number;
  /** Modalidade de recolhimento salva no cálculo */
...
function calcularFaixas(
  ...
  return receitas.map(({ descricao, aliquota, valor }) => {
    ...
    const parcelaSemAcrescimo   = excedente > 0 ? proporcao * LIMITE_SEM_MAJORACAO : valor;
    ...
    return { descricao, aliquota, aliquotaAcrescimo, valor, proporcao, parcelaSemAcrescimo, parcelaComAcrescimo, baseCalculo };
  });
}

export function calcular(entrada: EntradaCalculo, opcoes: OpcaoCalculo = {}): ResultadoCalculo {
  ...
  // Permite override; padrão é APLICAR a majoração na CSLL
  ...
  // ── IRPJ: excedente condicionado à flag aplicarMajoracaoIrpj ─────────────
  const excedenteIrpj = aplicarMajoracaoIrpj ? excedente : 0;

  // ── IRPJ: sempre aplica o excedente ──────────────────────────────────────
  const linhasIrpj = calcularFaixas([
  ...
  // ── CSLL: excedente condicionado à regra 1T/2026 ─────────────────────────
  const exedenteCsll          = aplicarMajoracaoCsll ? excedente : 0;
  const csllMajoracaBloqueada = excedente > 0 && !aplicarMajoracaoCsll;
  const linhasCsll = calcularFaixas([
  ...
  return {
    ...
    linhasCsll, basePresumidaCsll, csll9, csllRetida, csllARecolher,
    csllMajoracaBloqueada,
    irpjMesesAnteriores, csllMesesAnteriores,
    irpjResidual, csllResidual, temPagamentosMensais,
